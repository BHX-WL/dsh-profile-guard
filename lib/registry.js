...
// chain carries the real reason (ECONNREFUSED, ENOTFOUND, ...). Walk the
// chain and surface the first error code so the user can tell a refused
// connection from a DNS miss instead of seeing only "fetch failed".
...
export async function fetchManifest(pkg, { registry = defaultRegistry(), timeoutMs = 10000 } = {}) {
  ...
  try {
    ...
    throw new Error(`guard: registry fetch failed for ${name} (${url}): ${networkFailureCode(e)}`);
