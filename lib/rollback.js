...
export async function rollbackToSnapshot(profile, snapshotId, { autoRestart = true, stopPort = true } = {}) {
  ...
  if (autoRestart && check.ok) {
    ...
    if (!restarted) {
      // hostLogTail(null) is always "" (no logFile captured) — fall back to an
      // explicit reason so "attempted and failed" is never reported as "未执行".
      restartError = restartErrorOf(hostLogTail(null));
    } else {
