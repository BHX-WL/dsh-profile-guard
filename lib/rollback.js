...
// plugin install broke the host boot. Options: autoRestart restarts and
// re-verifies the host after the restore; stopPort gates killing the host on
// 3080; crashReason carries the drained boot log (or any caller-provided
// reason) into the restore report under "崩溃原因". The restart, when it runs,
// drains into a crash/rollback-restart-<ts>.log so a failed restart is
// diagnosable. Flow: read the snapshot meta -> back the
...
export async function rollbackToSnapshot(profile, snapshotId, { autoRestart = true, stopPort = true, crashReason = "" } = {}) {
  ...
  // expected hash of the manifest as restored + reconciled: the re-pull
  // verification compares the on-disk manifest to this, not to meta.hash.
  const expectedHash = manifestHash(after);
  const check = staticCheck(profile);
  // 5) optional auto restart; afterwards verify the restored manifest survived the
  //    re-pull. The recheck baseline is the hash we actually wrote back after
  //    reconcile (expectedHash) — meta.hash is kept as a reference only, because
  //    reconcile can legitimately narrow the bundles to the restored deps.
  let restartError = null;
  let restarted = false;
  let externallyReset = false;
  let hashOk = null;
  if (autoRestart && check.ok) {
    // Drain the restarted host's output into a crash/ log so a restart that
    // fails has a real tail to diagnose with instead of only the generic
    // timeout message (hostLogTail(null) is always "").
    const restartLog = join(crashDir(profile), "rollback-restart-" + ts + ".log");
    mkdirSync(crashDir(profile), { recursive: true });
    // launch arg via the central contract constant (C1) so a host CLI rename
    // stays an env override (DSH_GUARD_NO_OPEN) away instead of a code change (I3)
    const child = spawnHost(profile, [contract.noOpenFlag()], { logFile: restartLog });
    child.unref();
    restarted = await waitHostReady("http://127.0.0.1:3080", 45000);
    if (!restarted) {
      // empty tail still falls back to an explicit reason so "attempted and
      // failed" is never reported as "未执行".
      restartError = restartErrorOf(hostLogTail(restartLog));
    } else {
      hashOk = manifestHash(readManifest(dir)) === expectedHash;
      if (!hashOk) {
  ...
  const report = buildRestoreReport({ profile, reason: "auto-rollback after boot failure", before, after, snapshotId, check, restarted, restartError, externallyReset, hashOk, stopErrors, crashReason });
  const file = saveRestoreReport(guardsDir(profile), report);
  return { ok: true, restored: true, restarted, restartError, externallyReset, hashOk, snapshotHash: meta.hash, report, file };
}

// A restart that fails without any captured host log must still report *why*: an
// empty tail means "no logFile was captured", not "restart was not attempted".
export function restartErrorOf(tail) {
  const t = String(tail ?? "").trim();
  return t ? t : "host did not become ready within 45000ms";
}
