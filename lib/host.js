...
export async function stopHostByPort(port = 3080, { dryRun = false } = {}) {
  ...
  try {
    ...
    for (const pid of pids) {
      ...
      if (/dsh|bin\.js|deepseek/i.test(cmd.replace(/\0/g, ""))) {
        if (!dryRun) execFileSync("taskkill", ["/PID", String(pid), "/T", "/F"], { windowsHide: true });
        killed.push(pid);
      }
    }
  } catch { /* nothing listening, netstat/wmic missing, or no dsh match */ }
  return { killed };
}
