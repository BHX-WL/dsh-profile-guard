...
// Drain a spawned host's stdout/stderr into <logFile> (append) so a later
// hostLogTail can see why a boot failed. The host is long-running and its pipes
// would otherwise sit unread; reading them keeps it from stalling and preserves
// the log for failure diagnosis. Best effort: a write error must never crash
// the guard, and a spawn error is still surfaced through waitHostReady.
export function attachHostLog(child, logFile) {
  if (!child || !logFile || !child.stdout || !child.stderr) return child;
  const out = createWriteStream(logFile, { flags: "a" });
  out.on("error", () => {}); // log is best-effort
  child.on("error", () => {});
  child.stdout.on("data", (c) => { out.write(c); });
  child.stderr.on("data", (c) => { out.write(c); });
  child.on("close", () => { try { out.end(); } catch {} });
  return child;
}
...
export function spawnHost(profile, extraArgs = [], { logFile = null } = {}) {
  ...
  const child = spawn(cmd, args, { stdio: ["ignore", "pipe", "pipe"], windowsHide: true, shell: false });
  // without logFile the pipes stay unread as before (backward compatible)
  return attachHostLog(child, logFile);
}
