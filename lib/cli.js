...
import { resolveDshBin } from "./host.js";
...
async function main() {
  ...
  switch (cmd) {
    ...
    case "preflight": {
      // Pre-install safety check (design 5): fetch the candidate manifest from
      // the registry, run the pure verdict engine against host facts and the
      // profile's installed insert ids, then map verdicts to exit codes: 0 ok
      // (or forced), 1 refused, 2 usage. All I/O here is real-safe: one registry
      // GET plus local reads - nothing destructive.
      const pkg = rest[0];
      ...
      // Warnings are visible signals by design (e.g. an offline bundle-patch
      // that could not be insert-id verified): print them to stderr whether
      // the run passes or refuses, so a green exit never hides a degraded check.
