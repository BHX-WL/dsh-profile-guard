...
async function main() {
  ...
  switch (cmd) {
    ...
    case "remote": {
      ...
      // Human output carries URL/token/verification/time (design §4). The
      // success path reaches here only with info.verified === true (stale or
      // unverified results exit via !info.ok above), so the line is a plain
      // confirmation, not a state probe.
      process.stdout.write(`guard: phone URL: ${shownUrl}\n`);
      process.stdout.write(`guard: token: ${shown}\n`);
      process.stdout.write(`guard: verified: yes\n`);
      process.stdout.write(`guard: at: ${at}\n`);
