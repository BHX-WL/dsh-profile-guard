...
async function main() {
  ...
  switch (cmd) {
    ...
    case "install": {
      ...
      if (!noBoot) {
        // Post-install verification: bootOnce reuses the task-1 rollback — on a
        // plugin-level boot failure it restores the latest healthy snapshot and
        // restarts; install surfaces that as a failure with the rollback note.
        const b = await bootOnce(profile, [], {});
        if (!b.ok) {
