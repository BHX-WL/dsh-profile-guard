  ...
  switch (cmd) {
    ...
    case "install": {
      ...
      if (!noBoot) {
        ...
        // rollback). I1 (final review): a rolled-back verification is a FAILED
        // install — the plugin was removed — whatever bootRollbackResult's ok
        // says (the host may have recovered on the healthy snapshot).
        ...
        if (outcome.exitCode !== 0) {
          ...
          // child pipes are still open: force-exit (fail) instead of draining
          // naturally. The preflight fetch settled long before this point, so
          // the S1 force-exit race does not apply here.
          fail(outcome.message, outcome.exitCode);
        }
        // Verification ok and the plugin stuck (no rollback): the host is up
        // with the new plugin — force-exit for the same pipe reason as above.
        process.stdout.write(`install ok (snapshot ${snap.id})\n`);
        process.exit(0);
      }
